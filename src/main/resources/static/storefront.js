...
async function initProduct() {
    ...
    try {
        ...
        addButton.addEventListener("click", async () => {
            ...
            try {
                state.cart = await api.addToCart(state.customerId, {
                    itemId: item.id || item.sku,
                    sku: item.sku,
                    itemName: item.itemName,
                    upc: item.upc,
            ...
            button.addEventListener("click", () => {
                ...
                switch (button.dataset.fulfillment) {
                    case "pickup":
                        ...
                        fulfillmentDetail.textContent = "Pick up today at ShopSmart Supercenter, 2825 N Ashland Ave, Chicago, IL.";
                        break;
                    default:
...
async function initCart() {
    ...
    async function render() {
        ...
        try {
            ...
            updateHomeCartPill();
            let selectedIds = getSelectedCartItems();
            if (!selectedIds.length) {
                selectedIds = items.map((item) => item.itemId);
                saveSelectedCartItems(selectedIds);
            }
            itemsNode.innerHTML = "";
            if (!items.length) {
    ...
    function updateCartSummary(items) {
        const selectedIds = getSelectedCartItems();
...
async function initCheckout() {
    ...
    const selection = () => new Set(getSelectedCartItems());
    syncSessionChrome();
    if (state.token) {
        await resolveSignedInAccount();
    }
    ...
    payNow.addEventListener("click", async () => {
        try {
            if (!state.order?.orderNumber) {
                state.order = await createOrderFromSelection(message, selection);
            }
            ...
            const submitted = await api.submitPayment({
                orderId: state.order.orderNumber,
                customerId: state.customerId,
                paymentMethod: selectedPaymentMethod,
                amount: state.order.totalAmount,
                currencyCode: state.order.currencyCode || "USD",
                idempotencyKey: `pay-${Date.now()}`,
                externalReference: `web-submit-${Date.now()}`
            });
            state.payment = await api.updatePayment(submitted.paymentNumber, {
                paymentStatus: "CAPTURED",
                externalReference: `web-capture-${Date.now()}`,
                gatewayResponseCode: "00",
                gatewayResponseMessage: "Approved"
            });
            state.order = await api.getOrder(state.order.orderNumber);
            ...
            state.lastPaymentNumber = state.payment.paymentNumber;
            saveSession();
            updateHomeCartPill();
            ...
            window.setTimeout(() => {
                window.location.href = `/confirmation.html?orderNumber=${encodeURIComponent(state.order.orderNumber)}&paymentNumber=${encodeURIComponent(state.payment.paymentNumber)}`;
            }, 800);
        } catch (error) {
            message.hidden = false;
            message.textContent = `Unable to capture payment: ${error.message}`;
        }
    });
    ...
    async function refreshCheckoutSummary() {
        ...
        const items = (cart.items || []).filter((item) => selectedIds.has(item.itemId));
        const subtotal = items.reduce((sum, item) => sum + Number(item.quantity || 0) * Number(item.unitPrice || 0), 0);
        applyShippingRule(subtotal);
...
async function createOrderFromSelection(messageNode, selectionGetter) {
    if (!state.token) {
        throw new Error("Sign in first.");
    }
    const cart = await api.getCart(state.customerId);
    const selectedIds = selectionGetter();
    const items = (cart.items || []).filter((item) => selectedIds.has(item.itemId));
    if (!items.length) {
    ...
    const payload = {
        ...
        shippingAmount: Number(document.getElementById("checkout-shipping").value || 0).toFixed(2),
        discountAmount: document.getElementById("checkout-order-discount").textContent.replace(/[^0-9.-]/g, "") || "0.00",
        shippingAddress: readAddress("ship"),
    ...
    async function render() {
        ...
        try {
            ...
            sortedAddresses.forEach((address) => {
                ...
                node.innerHTML = `
                    <h3>${escapeHtml(address.label || address.addressType || "Address")}${address.defaultAddress ? ` <span class="default-badge">Default</span>` : ""}</h3>
                    <div>${escapeHtml(address.recipientName || "")}</div>
                    <div class="muted">${escapeHtml([address.addressLine1, address.addressLine2, address.city, address.state, address.postalCode, address.country].filter(Boolean).join(", "))}</div>
            ...
            sortedPayments.forEach((payment) => {
                ...
                node.className = "account-card";
                node.innerHTML = `
                    <h3>${escapeHtml(payment.provider || payment.paymentMethodType || "Payment method")}${payment.defaultMethod ? ` <span class="default-badge">Default</span>` : ""}</h3>
                    <div>${escapeHtml(payment.maskedNumber || payment.accountToken || "")}</div>
                    <div class="muted">${escapeHtml(payment.cardholderName || "")}</div>
                    <div class="inline-actions">
                    ...
                    node.innerHTML = `
                        <button class="order-history-toggle" type="button">
                            <span class="stack-tight">
                                ...
                                <span class="muted">${escapeHtml(formatDate(order.createdAt))}</span>
                            </span>
